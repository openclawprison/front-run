...
const latestSourceValue = (trend: Trend) => {
  const timestamp = new Date(trend.latestSourceAt ?? "").getTime();
...
const categories = [
  { name: "All animals", icon: LayoutGrid, subs: [] as string[] },
  ...TREND_TAXONOMY[0].subcategories.map((name) => ({ name, icon: PawPrint, subs: [] as string[] })),
];
...
export default function Dashboard({ initialPayload }: { initialPayload: TrendsPayload | null }) {
  ...
  const trends = useMemo(() => (payload?.trends ?? []).filter((trend) => trend.category === "Animals"), [payload]);
  const visibleTrends = useMemo(() => {
    const normalizedQuery = query.trim().toLowerCase();
    const fallbackTime = payload ? new Date(payload.refreshedAt).getTime() : 0;
    return [...trends]
      .filter((trend) => category === "All animals" || trend.subcategory === category)
      ...
      .sort((a, b) => {
        if (sortMode === "newest") {
          const firstA = firstSeenValue(a, fallbackTime);
          const firstB = firstSeenValue(b, fallbackTime);
          const detectionMinuteDelta = Math.floor(firstB / 60_000) - Math.floor(firstA / 60_000);
          return detectionMinuteDelta || latestSourceValue(b) - latestSourceValue(a) || b.score[activeWindow] - a.score[activeWindow];
        }
  ...
  }, [activeWindow, acceleratingOnly, category, payload, query, sortMode, trends]);
    ...
    <div className="app-shell" data-feed-mode="server-persisted">
      <aside className="sidebar">
        <div className="brand-row">
          ...
          <div><div className="brand-name">FRONT RUN</div><div className="brand-sub">Animal signal intelligence</div></div>
        </div>

        <nav className="side-nav" aria-label="Trend categories">
          <p className="nav-label">Animal desk</p>
          {categories.map((item) => {
            const Icon = item.icon;
            const isActive = category === item.name;
            const animalSummary = payload?.categories?.find((candidate) => candidate.name === "Animals");
            const count = item.name === "All animals" ? trends.length : animalSummary?.subcategories.find((candidate) => candidate.name === item.name)?.count ?? trends.filter((trend) => trend.subcategory === item.name).length;
            return (
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <label className="search-box"><Search size={17} /><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search an animal, zoo or species" aria-label="Search animal news" /><kbd>Ctrl K</kbd></label>
        ...
        <div className="content-wrap">
          <section className="page-intro">
            <div><div className="eyebrow"><Flame size={14} /> Live animal desk</div><h1>Meet the animal<br />before it goes viral.</h1><p>Latest animal news, zoo arrivals, rescues and social breakouts—ranked by freshness and viral momentum.</p></div>
          ...
          {headlineTrends.length > 0 ? (
            <section className="hero-grid">
              <article className={`lead-signal tone-${headlineTrends[0].tone}`}>
                <div className="lead-topline"><span className="lead-rank">01 / leading animal story</span><PhasePill phase={headlineTrends[0].phase} /></div>
                <div className="lead-body"><div><div className="lead-category">{headlineTrends[0].subcategory}</div><h2>{headlineTrends[0].title}</h2><p>{headlineTrends[0].summary}</p></div><ScoreRing value={headlineTrends[0].score[activeWindow]} /></div>
              ...
              <div className="secondary-signals">
                {headlineTrends.slice(1).map((trend, index) => (
                  <button key={trend.id} className="signal-card" onClick={() => setSelectedTrend(trend)}>
                    ...
                    <div className="signal-card-title"><h3>{trend.title}</h3><ScoreRing value={trend.score[activeWindow]} compact /></div><p>{trend.subcategory}</p>
            ...
            <section className="empty-state"><LoaderCircle className="spin" size={24} /><h2>Reading animal sources</h2><p>Collecting the latest animal news and social activity now.</p></section>
          ) : (
          ...
          <div className="dashboard-grid">
            <section className="trend-board">
              <div className="section-heading"><div><span className="section-kicker">Latest animal news</span><h2>{categoryLabel}</h2></div><span className="result-count">{visibleTrends.length} stories</span></div>
            ...
            <aside className="insight-rail">
              ...
              <section className="news-panel">
                <div className="section-heading compact"><div><span className="section-kicker">Fresh coverage</span><h2>Latest animal news</h2></div><Newspaper size={18} /></div>
      ...
      {selectedTrend && (
        <div className="drawer-layer" role="dialog" aria-modal="true" aria-label={`${selectedTrend.title} analysis`}>
          ...
          <aside className="analysis-drawer">
            <div className="drawer-header"><div className="drawer-brand"><PawPrint size={16} /> Front Run animal analysis</div><button className="icon-button" onClick={() => setSelectedTrend(null)} aria-label="Close"><X size={19} /></button></div>
            <div className="drawer-content">
              <div className="drawer-title-row"><div className={`trend-mark large tone-${selectedTrend.tone}`}>{selectedTrend.mark}</div><div><span>{selectedTrend.subcategory}</span><h2>{selectedTrend.title}</h2></div></div>
              ...
              <section className="drawer-section">
                ...
                <div className="source-bars">{Object.entries(selectedTrend.sources).map(([source, value]) => <div key={source}><div><span>{source === "x" ? "X" : source.charAt(0).toUpperCase() + source.slice(1)}</span><strong>{value}%</strong></div><div className="source-track"><span style={{ width: `${value}%` }} /></div></div>)}</div>
              </section>
